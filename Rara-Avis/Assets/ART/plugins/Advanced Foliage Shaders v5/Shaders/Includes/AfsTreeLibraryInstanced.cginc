...
UNITY_INSTANCING_CBUFFER_START (Trees)
...
{
	float3 planeNormal = UNITY_ACCESS_INSTANCED_PROP(_SquashPlaneNormal).xyz;
	float3 projectedVertex = pos.xyz - (dot(planeNormal, pos.xyz) + UNITY_ACCESS_INSTANCED_PROP(_SquashPlaneNormal).w) * planeNormal + _AfsBillboardCameraForward.xyz * halfPI * pos.y;
...
{
	float3 planeNormal = UNITY_ACCESS_INSTANCED_PROP(_SquashPlaneNormal).xyz;
	float3 projectedVertex = normal.xyz - (dot(planeNormal, normal.xyz) + UNITY_ACCESS_INSTANCED_PROP(_SquashPlaneNormal).w) * planeNormal + _AfsBillboardCameraForward.xyz * halfPI * normal.y;
...
//	Adjust and fade in wind
	...
	float4 Wind = UNITY_ACCESS_INSTANCED_PROP (_Wind);
...
void AfsTreeVertBark_DepthNormal (inout appdata_full v)
{
	...
	TreeWorldPos.w = clamp(fadeState * UNITY_ACCESS_INSTANCED_PROP(_SquashAmount), 0.0, 1.0);
//	Scale
	v.vertex.xyz *= UNITY_ACCESS_INSTANCED_PROP(_TreeInstanceScale).xyz;
...
//	Store fadestate (delayed)
	...
	float shadowState = clamp(fadeState * UNITY_ACCESS_INSTANCED_PROP(_SquashAmount), 0.0, 1.0);

	#if defined(SURFACESHADER)
		o.AFSuv_MainTex.w = shadowState;
	//	Apply tree color
		v.color.rgb = lerp(_AfsTreeColor, 1.0, UNITY_ACCESS_INSTANCED_PROP(_TreeInstanceColor).g); // * _Color.rgb;
...
void AfsTreeVertLeaf_DepthNormal (inout appdata_full v)
{
	...
	shadowState = clamp(shadowState * UNITY_ACCESS_INSTANCED_PROP(_SquashAmount), 0.0, 1.0);
...
//	Decode UV3
	float3 pivot;
		...
		pivot *= v.texcoord2.y;
		pivot *= UNITY_ACCESS_INSTANCED_PROP(_TreeInstanceScale).xyz;
	#endif
//	Add extra animation to make it fit speedtree
	TreeWorldPos.xyz = abs(TreeWorldPos.xyz);
	float sinuswave = _SinTime.z;
...
//	Apply Wind
		...
		bendingCoords.ba = ( UNITY_ACCESS_INSTANCED_PROP(_BendingControls) == 0) ? v.color.bb : v.texcoord3.xy;
