...
Shader "Nature/Afs Tree Creator Leaves Optimized CTI" {
	Properties {
	...
	SubShader { 
		Tags {
			...
			"RenderType"="AFSTreeLeafCTI"
		}
		LOD 200
		CGPROGRAM
			// Use our own early alpha testing: so no alphatest:_Cutoff
			#pragma surface surf AFSSpecular vertex:AfsTreeVertLeaf nolightmap fullforwardshadows keepalpha
			#pragma target 3.0
			
		//	#define XLEAFBENDING
		//	Enable CTI features instead
			#define LEAFTUMBLING
			...
			//	Store fadestate (delayed)
				...
				float shadowState = saturate(fadeState * UNITY_ACCESS_INSTANCED_PROP(_SquashAmount));
